...
<body class="bg-blue-50">
    ...
    <div class="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-lg mt-10">
        <h1 class="text-3xl font-bold text-dark-800 mb-6">Modifier le Client</h1>
        <form action="{{ route('clients.update', $client->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-6">
                <label for="name" class="block text-lg font-medium">Nom</label>
                <input type="text" name="name" id="name" value="{{ $client->name }}" class="mt-2 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            </div>
            <div class="mb-6">
